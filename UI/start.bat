...
rem    3) 아두이노 시리얼 브리지 (backend\serial_bridge.py)      :8078
rem
...
echo.
echo   [1/3] 판정 모델 서버 여는 중 ... http://127.0.0.1:8077
start "waferQC 모델 서버" /D "%MODEL%" cmd /k .venv\Scripts\python.exe serve.py

echo   [2/3] 화면 개발 서버 여는 중 ... http://127.0.0.1:5180
start "waferQC 화면" /D "%FRONT%" cmd /k npm run dev

echo   [3/3] 아두이노 시리얼 브리지 시작 중 ... http://127.0.0.1:8078
start "waferQC 아두이노" /D "%ROOT%backend" cmd /k wafer_final_package_v2\.venv\Scripts\python.exe serial_bridge.py
